...
export class WindField {
  ...
  sample(out: THREE.Vector3, pos: THREE.Vector3, time: number): THREE.Vector3 {
    ...
    if (t.intensity <= 0) return out.set(0, 0, 0);
    ...
    const swirl = cfg.maxTangential * t.intensity * profile;
    const inflow = -cfg.inflowFactor * swirl; // negative = toward the funnel
    const updraft =
      cfg.updraftSpeed * t.intensity * Math.max(0, 1 - d / (cfg.updraftRadius * R));
    ...
    return out.set(
      (-rz * swirl + rx * inflow) * gust * hFade,
      updraft * gust * hFade,
      (rx * swirl + rz * inflow) * gust * hFade,
    );
  }
