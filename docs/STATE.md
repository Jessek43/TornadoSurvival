...
- Rapier heightfield collider agrees with heightAt on an asymmetric grid (heightfieldBuffer transpose cancels parry's column-major x↔z swap). `verify:axes`
...
| `verify:axes` | Rapier heightfield vs heightAt on an asymmetric planar grid (guards the heightfieldBuffer transpose) | 36/36 asymmetric, 0/50 missed, 0/50 mismatch |
